...
import { theme } from "../components/CodeSnippet/syntax-highlighting-theme";
...
export async function remarkCodeSnippets(): Promise<Plugin<[], Root>> {
  const makeVisitor =
    (format: "md" | "mdx", highlighter: Highlighter) =>
    (code: Code, index: number | null, parent) => {
      ...
      let codeSnippetHtml = code.value;
      try {
        codeSnippetHtml = highlighter.codeToHtml(
          code.value,
          code.lang
            ? {
                lang: code.lang,
        ...
        // lang is not recognized by shiki
        codeSnippetHtml = highlighter.codeToHtml(code.value);
      }
  ...
  const mdVisitor = makeVisitor("md", highlighter);
  const mdxVisitor = makeVisitor("mdx", highlighter);
...
function parseMeta(meta) {
  ...
  // Try to find the meta property `command="..."` or `command='...'`,
  // store its value and remove it from meta
  let command;
  meta = meta.replace(
    /(?:\s|^)command\s*=\s*(["'])(.*?)(?<!\\)\1/,
